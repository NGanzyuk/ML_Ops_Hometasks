...
from grpcService import model_service_pb2
from grpcService import model_service_pb2_grpc
...
def train_model():
    ...
    with get_grpc_channel() as channel:
        stub = model_service_pb2_grpc.ModelServiceStub(channel)
        response = stub.TrainModel(model_service_pb2.TrainRequest(model_type=data['model_type'], params=data['params']))
...
def get_prediction():
    ...
    with get_grpc_channel() as channel:
        stub = model_service_pb2_grpc.ModelServiceStub(channel)
        response = stub.GetPrediction(model_service_pb2.PredictionRequest(model_name=model_name, input_data=input_data))
...
def delete_model():
    ...
    with get_grpc_channel() as channel:
        stub = model_service_pb2_grpc.ModelServiceStub(channel)
        response = stub.DeleteModel(model_service_pb2.DeleteRequest(model_name=model_name))
        return jsonify({"status": response.status}), 200
